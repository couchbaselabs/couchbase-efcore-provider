    ...
    where TFixture : NorthwindQueryRelationalFixture<NoopModelCustomizer>, new()
{
        ...
        var query = context.Set<Product>().FromSqlRaw(
            NormalizeDelimitersInRawString(
                @"SELECT [ProductName] AS [ProductID], [ProductID] AS [ProductName], [SupplierID], [UnitPrice], [UnitsInStock], [Discontinued]
                      FROM [Content].[Northwind].[Products]"));
    ...
    public virtual async Task Bad_data_error_handling_invalid_cast_no_tracking(bool async)
    {
        ...
        var query = context.Set<Product>()
            .FromSqlRaw(
                NormalizeDelimitersInRawString(
                    @"SELECT [ProductName] AS [ProductID], [ProductID] AS [ProductName], [SupplierID], [UnitPrice], [UnitsInStock], [Discontinued]
                    FROM [Content].[Northwind].[Products]")).AsNoTracking();
    ...
    public virtual Task FromSqlRaw_queryable_simple(bool async)
        => AssertQuery(
            ...
            ss => ((DbSet<Customer>)ss.Set<Customer>())
                .FromSqlRaw(NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers] WHERE [ContactName] LIKE '%z%'")),
    ...
    public virtual Task FromSqlRaw_queryable_simple_columns_out_of_order(bool async)
        => AssertQuery(
            ...
            ss => ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
                NormalizeDelimitersInRawString(
                    "SELECT [Region], [PostalCode], [Phone], [Fax], [CustomerID], [Country], [ContactTitle], [ContactName], [CompanyName], [City], [Address] FROM [Content].[Northwind].[Customers]")),
            ss => ss.Set<Customer>());
    ...
    public virtual Task FromSqlRaw_queryable_simple_columns_out_of_order_and_extra_columns(bool async)
        => AssertQuery(
            ...
            ss => ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
                NormalizeDelimitersInRawString(
                    "SELECT [Region], [PostalCode], [PostalCode] AS [Foo], [Phone], [Fax], [CustomerID], [Country], [ContactTitle], [ContactName], [CompanyName], [City], [Address] FROM [Content].[Northwind].[Customers]")),
            ss => ss.Set<Customer>());
    ...
    public virtual async Task FromSqlRaw_queryable_simple_columns_out_of_order_and_not_enough_columns_throws(bool async)
    {
        ...
        var query = context.Set<Customer>().FromSqlRaw(
            NormalizeDelimitersInRawString(
                "SELECT [PostalCode], [Phone], [Fax], [CustomerID], [Country], [ContactTitle], [ContactName], [CompanyName], [City], [Address] FROM [Content].[Northwind].[Customers]"));
    ...
    public virtual Task FromSqlRaw_queryable_composed(bool async)
        => AssertQuery(
            async,
            ss => ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers]"))
    ...
    public virtual Task FromSqlRaw_queryable_composed_after_removing_whitespaces(bool async)
        => AssertQuery(
            ...
            ss => ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
                    NormalizeDelimitersInRawString(
                        _eol + "    " + _eol + _eol + _eol + "SELECT" + _eol + "* FROM [Content].[Northwind].[Customers]"))
                .Where(c => c.ContactName.Contains("z")),
            ss => ss.Set<Customer>().Where(c => c.ContactName.Contains("z")));
        ...
        if (async)
        {
            var query = EF.CompileAsyncQuery(
                (NorthwindContext context) => context.Set<Customer>()
                    .FromSqlRaw(NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers]"))
                    .Where(c => c.ContactName.Contains("z")));
    ...
    public virtual async Task FromSqlRaw_queryable_composed_compiled_with_parameter(bool async)
    {
        if (async)
        {
            var query = EF.CompileAsyncQuery(
                (NorthwindContext context) => context.Set<Customer>()
                    .FromSqlRaw(
                        NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers] WHERE [CustomerID] = {0}"), "CONSH")
                    .Where(c => c.ContactName.Contains("z")));
    ...
    public virtual async Task FromSqlRaw_queryable_composed_compiled_with_DbParameter(bool async)
    {
        if (async)
        {
            var query = EF.CompileAsyncQuery(
                (NorthwindContext context) => context.Set<Customer>()
                    .FromSqlRaw(
                        NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers] WHERE [CustomerID] = @customer"),
    ...
    public virtual async Task FromSqlRaw_queryable_composed_compiled_with_nameless_DbParameter(bool async)
    {
        if (async)
        {
            var query = EF.CompileAsyncQuery(
                (NorthwindContext context) => context.Set<Customer>()
                    .FromSqlRaw(
                        NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers] WHERE [CustomerID] = {0}"),
    ...
    public virtual Task FromSqlRaw_composed_contains(bool async)
        => AssertQuery(
            ...
            ss => from c in ss.Set<Customer>()
                  where ((DbSet<Order>)ss.Set<Order>()).FromSqlRaw(NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Orders]"))
    ...
    public virtual Task FromSqlRaw_composed_contains2(bool async)
        => AssertQuery(
            ...
            ss => from c in ss.Set<Customer>()
                  where
                      c.CustomerID == "ALFKI"
                      && ((DbSet<Order>)ss.Set<Order>()).FromSqlRaw(NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Orders]"))
                      .Select(o => o.CustomerID)
                      .Contains(c.CustomerID)
                  select c,
            ss => from c in ss.Set<Customer>()
    ...
    public virtual Task FromSqlRaw_queryable_multiple_composed(bool async)
        => AssertQuery(
            async,
            ss => from c in ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers]"))
                  from o in ((DbSet<Order>)ss.Set<Order>()).FromSqlRaw(NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Orders]"))
        ...
        return AssertQuery(
            ...
            ss => from c in ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers]"))
                  from o in ((DbSet<Order>)ss.Set<Order>()).FromSqlRaw(
                      NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Orders] WHERE [OrderDate] BETWEEN {0} AND {1}"),
                      startDate,
                      endDate)
                  where c.CustomerID == o.CustomerID
                  select new { c, o },
    ...
    public virtual async Task FromSqlRaw_queryable_multiple_composed_with_parameters_and_closure_parameters(bool async)
    {
        ...
        await AssertQuery(
            ...
            ss => from c in ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
                      NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = {0}"), city)
                  from o in ((DbSet<Order>)ss.Set<Order>()).FromSqlRaw(
                      NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Orders] WHERE [OrderDate] BETWEEN {0} AND {1}"),
                      startDate,
                      endDate)
    ...
    public virtual Task FromSqlRaw_queryable_multiple_line_query(bool async)
        => AssertQuery(
            ...
            ss => ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
                NormalizeDelimitersInRawString(
                    @"SELECT *
FROM [Content].[Northwind].[Customers]
WHERE [City] = 'London'")),
...
FROM [Content].[Northwind].[Customers]"))
    ...
    public virtual Task FromSqlRaw_queryable_with_parameters(bool async)
    {
        ...
        return AssertQuery(
            async,
            ss => ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
                NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = {0} AND [ContactTitle] = {1}"), city,
                contactTitle),
    ...
    public virtual Task FromSqlRaw_queryable_with_parameters_inline(bool async)
        => AssertQuery(
            async,
            ss => ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
                NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = {0} AND [ContactTitle] = {1}"), "London",
        ...
        return AssertQuery(
            ...
            ss => ((DbSet<Customer>)ss.Set<Customer>()).FromSqlInterpolated(
                NormalizeDelimitersInInterpolatedString(
                    $"SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = {city} AND [ContactTitle] = {contactTitle}")),
            ss => ss.Set<Customer>().Where(x => x.City == city && x.ContactTitle == contactTitle));
    }
    ...
    public virtual Task FromSqlInterpolated_queryable_with_parameters_inline_interpolated(bool async)
        => AssertQuery(
            ...
            ss => ((DbSet<Customer>)ss.Set<Customer>()).FromSqlInterpolated(
                NormalizeDelimitersInInterpolatedString(
                    $"SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = {"London"} AND [ContactTitle] = {"Sales Representative"}")),
            ss => ss.Set<Customer>().Where(x => x.City == "London" && x.ContactTitle == "Sales Representative"));
        ...
        await AssertQuery(
            ...
            ss => from c in ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
                  ...
                  from o in ((DbSet<Order>)ss.Set<Order>()).FromSqlInterpolated(
                      NormalizeDelimitersInInterpolatedString(
                          $"SELECT * FROM [Content].[Northwind].[Orders] WHERE [OrderDate] BETWEEN {startDate} AND {endDate}"))
                  where c.CustomerID == o.CustomerID
                  select new { c, o },
            ss => from c in ss.Set<Customer>().Where(x => x.City == city)
                  from o in ss.Set<Order>().Where(x => x.OrderDate >= startDate && x.OrderDate <= endDate)
    ...
    public virtual async Task FromSqlRaw_queryable_simple_cache_key_includes_query_string(bool async)
    {
        await AssertQuery(
            async,
            ss => ((DbSet<Customer>)ss.Set<Customer>())
                .FromSqlRaw(NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = 'London'")),
            ss => ss.Set<Customer>().Where(x => x.City == "London"));

        await AssertQuery(
            async,
            ss => ((DbSet<Customer>)ss.Set<Customer>())
                .FromSqlRaw(NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = 'Seattle'")),
    ...
    public virtual async Task FromSqlRaw_queryable_with_parameters_cache_key_includes_parameters(bool async)
    {
        var city = "London";
        var contactTitle = "Sales Representative";
        var sql = "SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = {0} AND [ContactTitle] = {1}";
...
AND (([UnitsInStock] + [UnitsOnOrder]) < [ReorderLevel])"))
    ...
    public virtual async Task FromSqlRaw_with_dbParameter(bool async)
    {
        ...
        await AssertQuery(
            async,
            ss => ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
                NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = @city"), parameter),
            ss => ss.Set<Customer>().Where(x => x.City == "London"));
    }
    ...
    public virtual async Task FromSqlRaw_with_dbParameter_mixed(bool async)
    {
        ...
        await AssertQuery(
            ...
            ss => ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
                NormalizeDelimitersInRawString(
                    "SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = {0} AND [ContactTitle] = @title"), city, titleParameter),
        ...
        await AssertQuery(
            ...
            ss => ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
                NormalizeDelimitersInRawString(
                    "SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = @city AND [ContactTitle] = {1}"), cityParameter, title),
            ss => ss.Set<Customer>().Where(x => x.City == city && x.ContactTitle == title));
    }
    ...
    public virtual async Task FromSqlRaw_with_db_parameters_called_multiple_times(bool async)
    {
        ...
        var query = context.Customers.FromSqlRaw(
            NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers] WHERE [CustomerID] = @id"), parameter);
    ...
    public virtual Task FromSqlRaw_with_SelectMany_and_include(bool async)
        => AssertQuery(
            ...
            ss => from c1 in ((DbSet<Customer>)ss.Set<Customer>())
                      .FromSqlRaw(NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers] WHERE [CustomerID] = 'ALFKI'"))
                  from c2 in ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
                          NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers] WHERE [CustomerID] = 'AROUT'"))
                      .Include(c => c.Orders)
                  select new { c1, c2 },
        ...
        => AssertQuery(
            ...
            ss => from c in ((DbSet<Customer>)ss.Set<Customer>())
                  ...
                  join o in ((DbSet<Order>)ss.Set<Order>()).FromSqlRaw(
                          NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Orders] WHERE [OrderID] <> 1"))
    ...
    public virtual async Task FromSqlInterpolated_with_inlined_db_parameter(bool async)
    {
        ...
        await AssertQuery(
            ...
            ss => ((DbSet<Customer>)ss.Set<Customer>())
                .FromSqlInterpolated(
                    NormalizeDelimitersInInterpolatedString($"SELECT * FROM [Content].[Northwind].[Customers] WHERE [CustomerID] = {parameter}")),
    ...
    public virtual async Task FromSqlInterpolated_parameterization_issue_12213(bool async)
    {
        ...
        var query1 = context.Orders
            .FromSqlInterpolated(NormalizeDelimitersInInterpolatedString($"SELECT * FROM [Content].[Northwind].[Orders] WHERE [OrderID] >= {min}"))
            .Select(i => i.OrderID);
        ...
        var query3 = context.Orders
            .Where(
                o => o.OrderID <= max
                    && context.Orders
                        .FromSqlInterpolated(
                            NormalizeDelimitersInInterpolatedString($"SELECT * FROM [Content].[Northwind].[Orders] WHERE [OrderID] >= {min}"))
                        .Select(i => i.OrderID)
                        .Contains(o.OrderID))
            .Select(o => o.OrderID);
    ...
    public virtual Task Entity_equality_through_fromsql(bool async)
        => AssertQuery(
            ...
            ss => ((DbSet<Order>)ss.Set<Order>())
                .FromSqlRaw(NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Orders]"))
        ...
        => AssertQuery(
            ...
            ss => ((DbSet<Customer>)ss.Set<Customer>())
                ...
                .Concat(
                    ...
                    .FromSqlRaw(NormalizeDelimitersInRawString("SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = 'Berlin'"))),
            ss => ss.Set<Customer>().Where(x => x.City == "London")
    ...
    public virtual Task Keyless_entity_with_all_nulls(bool async)
        => AssertQuery(
            ...
            ss => ((DbSet<OrderQuery>)ss.Set<OrderQuery>())
                .FromSqlRaw(NormalizeDelimitersInRawString("SELECT NULL AS [CustomerID] FROM [Content].[Northwind].[Customers] WHERE [City] = 'Berlin'"))
                .IgnoreQueryFilters(),
    ...
    public virtual Task Line_endings_after_Select(bool async)
        => AssertQuery(
            async,
            ss => ((DbSet<Customer>)ss.Set<Customer>())
                .FromSqlRaw(NormalizeDelimitersInRawString("SELECT" + Environment.NewLine + "* FROM [Content].[Northwind].[Customers]"))
        ...
        => AssertQuery(
            ...
            ss => ss.Set<Order>().Where(
                o => ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
                        NormalizeDelimitersInRawString(@"SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = @city"),
        ...
        => AssertQuery(
            ...
            ss => ss.Set<Order>().Where(
                o => ((DbSet<Customer>)ss.Set<Customer>())
                    .FromSqlRaw(
                        NormalizeDelimitersInRawString(@"SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = {0}"),
                        // ReSharper disable once FormatStringProblem
    ...
    public virtual async Task FromSqlRaw_with_dbParameter_mixed_in_subquery(bool async)
    {
        ...
        await AssertQuery(
            ...
            ss => ss.Set<Order>().Where(
                o => ((DbSet<Customer>)ss.Set<Customer>())
                    .FromSqlRaw(
                        NormalizeDelimitersInRawString(@"SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = {0} AND [ContactTitle] = @title"),
                        city,
                        // ReSharper disable once FormatStringProblem
        ...
        await AssertQuery(
            ...
            ss => ss.Set<Order>().Where(
                o => ((DbSet<Customer>)ss.Set<Customer>())
                    .FromSqlRaw(
                        NormalizeDelimitersInRawString(@"SELECT * FROM [Content].[Northwind].[Customers] WHERE [City] = @city AND [ContactTitle] = {1}"),
                        // ReSharper disable once FormatStringProblem
        ...
        => AssertQuery(
            ...
            ss => ((DbSet<Customer>)ss.Set<Customer>())
                .FromSqlRaw(
                    NormalizeDelimitersInRawString(
                        @"WITH [Customers2] AS (
    SELECT * FROM [Content].[Northwind].[Customers]
)
