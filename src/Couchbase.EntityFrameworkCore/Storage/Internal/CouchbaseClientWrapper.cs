...
public class CouchbaseClientWrapper : ICouchbaseClientWrapper
{
    ...
    public async Task<bool> CreateDocument<TEntity>(string id, (string? scope, string? collection) keyspace, TEntity entity)
    {
        ...
        try
        {
            _logger.LogDebug("Using keyspace {BucketName}.{keyspace.scope}.{keyspace.collection} for document {id}", 
                _couchbaseDbContextOptionsBuilder.Bucket, keyspace.scope, keyspace.collection, id);
