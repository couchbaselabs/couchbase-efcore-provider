...
2. **`CouchbaseDbDataReader<T>.GetValue(int ordinal)`** — when `_columnNames` is set, translates the shaper ordinal to a JSON property via `_fieldOrdinals.TryGetValue(colName, out var jsonOrdinal)`. `_fieldOrdinals` is a `Dictionary<string, int>` with `OrdinalIgnoreCase` — camelCase projection aliases from EF Core match camelCase JSON keys from Couchbase documents without any casing transform. Fields absent from the N1QL response (MISSING) return `DBNull.Value`. The positional fallback (no `_columnNames`) is preserved for scalar queries where ordinal access is correct.
...
| `Storage/Internal/CouchbaseDbDataReader.cs` | ✅ Phase 3 | `InitializeFieldInfo` handles scalar non-Object `JsonElement`; `GetOrdinal` falls back to 0 for scalar results; `GetValue` uses `_fieldOrdinals` (OrdinalIgnoreCase) for name-based ordinal translation when `_columnNames` is set |
