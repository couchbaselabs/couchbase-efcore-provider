...
* Finally, DbContext.SaveChangesAsync() is called and the entities modified properties are persisted to the database.

> [!NOTE]
> This provider is asynchronous-only — synchronous APIs like `SaveChanges()`/`.Single()` throw `NotSupportedException` in Release builds. Use the `Async` variants throughout, as in the examples on this page. See [Limitations](limitations.md#asynchronous-io-only).

## Approach 2: Bulk update with ExecuteUpdateAsync and ExecuteDeleteAsync

SaveChangesAsync is a powerful model for persisting changes to the database, but has some limitations. The main limitation is that each entity must be tracked and when SaveChangesAsync is called, each modified or added entity will be persisted to the database. This can be inefficient; a more efficient way of doing this is via an UPDATE or DELETE command.
...
More information on the differences between SaveChanges and ExecuteDelete and ExecuteUpdate can be found in the [main EF Core documentation](https://learn.microsoft.com/en-us/ef/core/saving/).

## Adding Data
Use the DbSet<TEntity>.Add method to add new instances of your entity classes. The data will be inserted into the database when you call DbContext.SaveChangesAsync():
...
> [!NOTE] 
> Single-document adds, updates, and removes made through change tracking (`Add`/`Update`/`Remove` + `SaveChangesAsync`) go through the [Couchbase Key/Value store](https://docs.couchbase.com/dotnet-sdk/current/howtos/kv-operations.html) rather than SQL++, since they operate on a single document by key. Bulk `ExecuteUpdate`/`ExecuteDelete` (above), raw `FromSqlRaw`/`FromSql` queries, and regular LINQ queries all go through SQL++.
...
`SaveChanges` latency doesn't scale linearly with the number of changed entities. This means
...
predictable prefix), rather than none of them. If you need all-or-nothing semantics for a group
of writes, use a [Couchbase transaction](transactions.md) instead of relying on ordering within a
...
> [!TIP] 
> If you attempt to delete the Blog after creating it, you will get a foriegn key constraint when you call SaveChangesAsync again, unless you provide DeleteBehavior.Cascade to the entity during modeling.
...
Eager loading via `Include`/`ThenInclude` is supported, including chained `ThenInclude`, `AutoInclude`, filtered includes, and includes on many-to-many skip navigations — see [Modeling](modeling.md#eager-loading) for the full set of examples. For instance, to load a blog with both its posts and its owner in one query:
```
